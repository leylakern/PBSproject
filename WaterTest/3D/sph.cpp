...
void sph::computeDensity(int i)//VectorXd& rho,int i,MatrixX2d& x,MatrixXi& neighbours)
{
    ...
    for (int j=0; j<N; ++j)
    {
        if ((neighbours(i,j)==1)&& (j!=i))
        {
            ...
            temp(1)=x(i,1)-x(j,1);
            temp(2)=x(i,2)-x(j,2);
            rho_temp+=m*w(temp.norm()); //check with ,neighobours' that other particle is within i.a. radius
        }
...
void sph::computeForce(int i)//,MatrixX2d& f,VectorXd& p,MatrixX2d& u, MatrixX2d& x,VectorXd& rho,MatrixXi& neighbours)
{
    ...
    for (int j=0; j<N; ++j)
    {
        if ((neighbours(i,j)==1)&& (j!=i))
        {
            ...
            f_p(i,1)+= (p(i)+p(j))*m/(rho(j)*2.)*std::abs(x(i,1)-x(j,1))*w_grad(temp.norm());//std::abs(x(i,1)-x(j,1)));
            f_p(i,0)+= (p(i)+p(j))*m/(rho(j)*2.)*std::abs(x(i,0)-x(j,0))*w_grad(temp.norm());
            
            f_p(i,2)+=(p(i)+p(j))*m/(rho(j)*2.)*std::abs(x(i,2)-x(j,2))*w_grad(temp.norm());
